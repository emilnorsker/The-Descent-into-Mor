...
func die(trigger_side_effects := true) -> void:
	...
	if get_map_data().player == entity:
		...
		death_message_color = ColorPallete.PLAYER_DIE
		SignalBus.player_died.emit()
	else:
		death_message = "%s is dead!" % entity.get_entity_name()
		death_message_color = ColorPallete.ENEMY_DIE
	
	if trigger_side_effects:
		SignalBus.message_sent.emit(death_message, death_message_color)
		get_map_data().player.level_component.add_xp(xp_given)
