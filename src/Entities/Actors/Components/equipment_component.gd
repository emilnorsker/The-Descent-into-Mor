...
func _equip_to_slot(slot: EquippableComponent.EquipmentType, item: Entity, add_message: bool) -> void:
	...
	if add_message:
		SignalBus.message_sent.emit("You equip the %s." % item.get_entity_name(), Color.WHITE)
...
func _unequip_from_slot(slot: EquippableComponent.EquipmentType, add_message: bool) -> void:
	...
	if add_message:
		SignalBus.message_sent.emit("You remove the %s." % current_item.get_entity_name(), Color.WHITE)
