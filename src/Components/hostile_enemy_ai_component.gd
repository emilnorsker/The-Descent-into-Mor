...
const MovementAction = preload("res://src/Actions/movement_action.gd")
const WaitAction = preload("res://src/Actions/wait_action.gd")
...
func perform() -> void:
	...
	if GameMap.get_tile(entity.grid_position).is_in_view:
		if distance <= 1:
	...
	if not path.is_empty():
		...
		if GameMap.get_blocking_entity_at_location(destination):
